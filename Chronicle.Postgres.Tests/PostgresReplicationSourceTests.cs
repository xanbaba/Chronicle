...
public class PostgresReplicationSourceTests(PostgresReplicationFixture fixture, ITestOutputHelper output)
    : IClassFixture<PostgresReplicationFixture>
{
    ...
    public async Task Insert_ShouldProduceInsertChangeEventWithCorrectFields()
    {
        ...
        try
        {
            await fixture.Container.ExecScriptAsync($"CREATE TABLE {tableName} (id int PRIMARY KEY, first_name VARCHAR(100), created_at TIMESTAMP);");
            ...
            var replicationOptions =
                new PostgresReplicationOptions(streamingConnectionString, slotName, PostgresReplicationFixture.PublicationName);
            ...
            var streamingTask = Task.Run(async () =>
            {
                ...
                await foreach (var rawChangeEvent in replicationSource.StreamAsync(replicationOptions, cts.Token))
                {
                    try
                    {
                        ...
                        var actualCreatedAt = DateTime.SpecifyKind(
                            DateTime.ParseExact(
                                rawChangeEvent.After["created_at"]!.ToString()!,
                                "yyyy-MM-dd HH:mm:ss",
                    ...
                    {
                        // clean up
                        await replicationSource.ConfirmAsync(rawChangeEvent.Offset, cts.Token);
                    }
                }
            ...
            var execResult = await fixture.Container.ExecScriptAsync($"INSERT INTO {tableName} VALUES (1, 'John', '{insertCreatedAt:O}');");
